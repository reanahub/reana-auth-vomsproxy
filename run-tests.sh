...
docker_build() {
    docker build -t docker.io/reanahub/reana-auth-vomsproxy .
}

docker_test() {
    docker run -i --rm docker.io/reanahub/reana-auth-vomsproxy voms-proxy-info 2>&1 | grep "Proxy not found"
}
